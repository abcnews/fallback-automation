...
import { DEFAULT_VIEWPORT_HEIGHT, DEFAULT_VIEWPORT_WIDTH, getChromeConfig } from './config';
...
async function getBodyHeight(page: puppeteer.Page) {
  ...
  return Math.ceil(fullPageSize.height);
}

async function getScreenshots({ url, width, fileType, selectors }: ScreenshotOptions) {
  ...
  const files = await selectors.reduce<Promise<Buffer[]>>(async (files, selector) => {
